  ...
  <div class="flex flex-col bg-white">
    <div class="flex-1 relative p-1">
      ...
      <div v-if="loading" class="absolute inset-0 flex flex-col items-center justify-center bg-white bg-opacity-90">
        ...
        <span class="mt-3 text-gray-700">Îß??∞Ïù¥?∞Î? Î∂àÎü¨?§Îäî Ï§?..</span>
      </div>
    </div>
  </div>
</template>
...
const robotPositions = ref(new Map()) // ?¨Îü¨ Î°úÎ¥á???ÑÏπòÎ•??Ä?•Ìïò??Map
...
const chartOption = computed(() => {
  ...
  // Î™®Îì† Î°úÎ¥á ?ÑÏπò ?úÏãú
  if (robotPositions.value) {
    robotPositions.value.forEach((position, robotSeq) => {
      // position??Ï°¥Ïû¨?òÍ≥† x, yÍ∞íÏù¥ Î™®Îëê ?àÎäî Í≤ΩÏö∞?êÎßå Ï≤òÎ¶¨
      if (position && 
          position.x != null && 
          position.y != null &&
          !isNaN(position.x) && 
      ...
      formatter: function(params) {
        ...
        try {
          ...
          if (params.seriesIndex < robotSeries.length && robotPositions.value) {
            ...
            return robot ? `Î°úÎ¥á: ${robot.nickname || robot.manufactureName || robotSeq}` : '';
          }
          
          // ?∏Îìú ?úÎ¶¨Ï¶àÏù∏ Í≤ΩÏö∞ - monitoring modeÍ∞Ä ?ÑÎãê ?åÎßå Ï¢åÌëú ?úÏãú
          if (!props.isMonitoringMode && params.componentSubType === 'scatter' && Array.isArray(params.data)) {
            ...
            return `Ï¢åÌëú: (${x.toFixed(2)}, ${y.toFixed(2)})`;
          }
    ...
    if (position && 
      ...
      seriesData.push({
        ...
        tooltip: {
          formatter: () => {
            const robot = robotsStore.robots.find(r => r.seq === robotSeq);
            return `Î°úÎ¥á: ${robot?.nickname || robot?.manufactureName || robotSeq}`;
          }
        }
      });
    }
...
function setupSSE() {
  // Í∏∞Ï°¥ ?∞Í≤∞???ïÎ¶¨
  if (robotPositions.value) {
    robotPositions.value.clear() // Í∏∞Ï°¥ ?ÑÏπò ?ïÎ≥¥??Ï¥àÍ∏∞??
  }
  if (eventSources) {
  ...
  // seqÍ∞Ä 1Í≥?2??Î°úÎ¥á???Ä?¥ÏÑúÎß?SSE ?§Ï†ï
  ...
  activeRobots.forEach(robot => {
    console.log(`Setting up SSE for robot ${robot.seq}`) // ?îÎ≤ÑÍπÖÏö©
    ...
    eventSource.onmessage = (event) => {
      try {
        ...
        console.error(`SSE message parsing error (Î°úÎ¥á ${robot.seq}):`, error)
      }
    }

    eventSource.onerror = (error) => {
      console.error(`SSE ?∞Í≤∞ ?êÎü¨ (Î°úÎ¥á ${robot.seq}):`, error)
      eventSource.close()
      robotPositions.value.delete(robot.seq) // ?êÎü¨ ???ÑÏπò ?ïÎ≥¥????†ú
    }
...
function handleNodeClick(params) {
  // Î™®Îãà?∞ÎßÅ Î™®Îìú?êÏÑú???∏Îìú ?¥Î¶≠ ÎπÑÌôú?±Ìôî
...
async function fetchMapData() {
  try {
    ...
    console.error('Îß??∞Ïù¥??Î°úÎî© ?§Ìå®:', error)
  } finally {
    loading.value = false
  }
}
...
// robotsStore.robotsÍ∞Ä Î≥ÄÍ≤ΩÎê† ??SSE ?¨ÏÑ§??
...
// currentRobotSeq Î≥ÄÍ≤??úÏóê???†ÌÉù??Î°úÎ¥áÎß??ÖÎç∞?¥Ìä∏
...
onUnmounted(() => {
  console.log('Cleaning up SSE connections...') // ?îÎ≤ÑÍπÖÏö©
  eventSources.forEach(source => {
    source.close()
    console.log('Closed SSE connection') // ?îÎ≤ÑÍπÖÏö©
  })
  eventSources.clear()
  robotPositions.value.clear() // ?ÑÏπò ?ïÎ≥¥???ïÎ¶¨
})
