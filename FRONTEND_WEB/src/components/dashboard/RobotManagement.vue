            ...
            <table class="min-w-full table-auto">
              <thead>
                <tr class="bg-gray-100">
                  ...
                  <th class="table-header-cell w-24">관리</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="robot in robots" :key="robot.seq" class="border-b border-gray-200 hover:bg-gray-50 text-center">
                  ...
                  <td class="table-data-cell">{{ robot.position ? `x: ${formatCoordinate(robot.position.x)}, y: ${formatCoordinate(robot.position.y)}` : '위치 없음' }}</td>
...
const handleBreakdown = async (robotSeq) => {
  try {
    ...
    if (robot && newStatus) robot.status = newStatus;
  } catch (error) {
...
const getStatusClass = (status) => {
  const map = {
    charging: 'bg-green-500', navigating: 'bg-blue-500',
    patrolling: 'bg-blue-500', emergencyStopped: 'bg-red-500',
    error: 'bg-red-600', waiting: 'bg-gray-500', homing: 'bg-teal-500',
  };
