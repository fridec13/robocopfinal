          ...
          <ul class="space-y-1 px-2">
            ...
            >
              ...
              >
                ...
                <span v-if="robotsStore.selectedRobot === robot.seq" class="text-green-400">
...
const robots = computed(() => {
  return robotsStore.displayRobots.slice().sort((a, b) => {
    return (isActive(b) ? 1 : 0) - (isActive(a) ? 1 : 0);
  });
});
...
onMounted(() => {
  ...
  if (savedRobot) robotsStore.selectedRobot = parseInt(savedRobot, 10)
  robotsStore.loadRobots()
})

const selectedRobotItem = computed(() =>
  robots.value.find(r => r.seq === robotsStore.selectedRobot) || null
)
